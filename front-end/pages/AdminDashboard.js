...
/**
 * Separator component to separate different aspects of a UI
 * @returns {JSX.Element}
 * @constructor
 */
...
/**
 * The Dashboard page for the administrator which composes of a calendar with indications of the days that have jobs
 * @returns {JSX.Element}
 * @constructor
 */
const AdminDashboard = () => {
  ...
  const DayCell = ({ date }, style) => {
    if (
      workDays.find(
        (workDay) => date.toISOString().split("T")[0] === workDay
      ) !== undefined
    ) {
      return (
        <View style={[styles.dayContainer, style.container]}>
          <View
            style={{
              backgroundColor: "rgba(194,194,194,0.33)",
              borderRadius: 10,
              borderWidth: 1,
              padding: 5,
              minWidth: 24,
          ...
          >
            <Text
              style={[style.text, styles.textDay]}
            >{`${date.getDate()}`}</Text>
          </View>
        </View>
      );
