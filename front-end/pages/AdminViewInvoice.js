...
import { Text, Divider, Datepicker } from "@ui-kitten/components";
    ...
    return (
      <View>
        ...
        <DataTable.Row onPress={handlePress}>
          <DataTable.Cell>{`${new Date(item["created_date"]).getMonth() + 1}-${
            new Date(item["created_date"]).getDate() + 1
          }-${new Date(item["created_date"]).getFullYear()}`}</DataTable.Cell>
